...
:wusd
索引类型: 索引在引擎内部实现,即便是相同类型的索引,实现方式可能不同
...
1. 最左前缀原则:
   1. 字段最左边的优先
   2. 字段的左前缀走索引
   3. 索引也会从左到右字段排序,满足了query~order by~group by和distinct才会使用索引
2. "选择性最大的字段放最左边":大部分好用,但是有可能有特殊情况搞崩系统
...
:wusd
重复和冗余索引: 用pt-duplicate-key-checker找到,然后删除掉就好
重复索引: 相同字段的BTree索引, 主键~唯一键已经创建了索引,容易重复创建; (A)和(A,ID)是重复索引.
冗余索引: 相同字段前缀是冗余索引
- 例子: (A,B)和(A)是冗余索引, 但是(A,B)和(B)不是冗余索引
- 最好继承索引,而不是增加索引.但是继承索引会使得速度慢,更占空间
```sql
-- 为了covering index, 用长的varchar继承integer索引,导致变慢
...
-- 假设有100万数据,某个state_id有20,000条数据
-- Q1: 115QPS
select count(*) from userinfo where state_id = 5;
-- Q2: < 10 QPS
-- 提升QPS,直接继承索引,来covering index. 结果Q2快了,但是Q1慢了
-- 如果想要都快,那么两个索引都要保留,缺点就是维护成本,插入慢了
...
---- 检查
1. 表腐败的现象: 重复主键,锁表,宕机
2. check table: 可以检查表和索引大部分错误
---- 修复
3. repair table: 可以修复腐败的表
4. 必要时备份,然后重新导入
---- 原因
5. 硬件错误(磁盘,内存错误) 可能
6. 移动了MySQL文件 最有可能
...
当优化器不知道多少行要检查,查询计划太复杂不知道多少行匹配,优化器会用索引统计数据去预估行数.analyze table会重新统计.
-- 重新统计表(索引部分)
...
碎片索引指的是叶子页没有顺序存储在磁盘上,但是范围扫描和全索引扫描会慢几倍.
- Row fragmentation: 行内部碎片.行存储分散
- Intra-row fragmentation: 行碎片.逻辑序列页或行没有顺序存储在磁盘上.
...
:wusd
索引碎片: 
- 索引碎片: 索引的叶子页没有顺序存储在磁盘上
- 缺点: 索引碎片会导致范围扫描和全索引扫描慢几倍
- 碎片类型:
  - 行内碎片: 一行存储在多个地方
  - 行碎片: 行存储的顺序和逻辑顺序不一致
  - 空间碎片: 分页很多空白
...
:wusd
创建索引和写查询时,牢记: 正好对应3-star法则
- 单行查询非常慢: 读一块数据只返回一行
- 访问有序数据很快:
  1. 顺序IO不需要查磁盘
  2. 不需要排序操作
- Index-only很快: 只需要查索引而不需要查行
索引的这些经验不可靠:
- "选择性最大的字段放多字段索引最左"
- "索引所有where条件字段"
如何知道索引好不好?
