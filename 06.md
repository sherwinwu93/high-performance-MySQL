...
int(1)==int(20)
:wusd
tinyint~smallint~mediumint~int~bigint 1~2~3~4~8B
tinyint: -128~127
...
:wusd
decimal不只用于存储准确小数,也可以用于存比bigint还大的整数.
float和double都不准确,一般用float(除非准确要求decimal),因为decimal损耗大.
## String Types
...
..
:wusd
VARCHAR一般占用空间比CHAR,因为只需要值的长度+描述长度.1byte255byte,2byte1000byte.
VARCHAR字段长度变化可能会导致分页.
所以字段变化少,最大长度大于平均长度,适合使用.
...
:wusd
其实文俊可以用JSON,而不是ES. JSON的字段可以用headerPath. 速度和MySQL几乎差不多
...
:wusd
主键字段是多个表的主键,在其他表里面类型也应该相同.
尽量选择最小类型的主键,因为不单单自己用,还有别的表也会用.
### Integer types
