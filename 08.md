...
2. 转化outer joins to inner join: 有时候外联因为where条件和表结构等于inner join
3. Evaluating and reducing constant expressions: 计算和减少常量表达式,在连表会常量传播
```sql
...
```
5. Applying algebraic equivalence rules:使用算术等式来简化算式 和 去除重复条件
   (5=5 and a > 5)->(a>5), ((a < b and b = c) and a = 5) -> b>5 and b=c and a =5
6. in() list comparisons: in的列表比较,先做排序,再二分查找
7. count(),min(),max() optimization: 直接用索引,不查表
   min直接用索引的最小值,max直接用索引的最大值. explain会显示Extra: Select tables optimized away.
8. Covering index: 覆盖索引,避免回表
9. Subquery optimization: 把一些子查询转换成索引查询(扫描索引,而不是表)
10. Early termination: 提前终止,比如LIMIT.比如不可能的情况1=0,film_id=-1.
...
- 表和索引统计数据:
  ...
  - 行或key的长度
  - key的分布信息
- 没有存储在server层,存储在engine层
- 用来选择最佳执行计划

#### MySQL's join execution strategy
MySQL认为一切都是join,包括单表查询,子查询.
MySQL执行UNION: 当作一系列单查,然后再把结果放入临时表,再读出来.每次查询都认为是join
以前用的是嵌套join,现在改用了hash join.

:wusd
MySQL执行join的策略:
1. 原来使用嵌套join,现在改用hash join(内存中)
2. MySQL执行UNION: 先单查,把结果放入临时表,再读,再查,join,再读.
...
MySQL的执行计划是命令的树结构.
```sql
explain analyze select film_id from film
    left join film_actor using(film_id)
                 where film_actor.actor_id is null;
```

:wusd
执行计划: MySQL的执行计划是命令的树结构,eg.连表时,左深树
```sql
-- 显示执行计划
explain analyze
select film_id from film
...
:wusd
join优化器: 最重要的优化器,优化join顺序
- N表连接,有N!种连表方式: optimizer_search_depth限制深度
- left join,不会做优化,因为其他表结果依赖上个表.
  除非必要,否则不用left join
...
:wusd
排序优化: 避免filesort 或 filesort更少的行
- filesort:
  + 无法根据索引产生排序结果,在磁盘或内存中排序,尽管没有使用文件,仍然叫做filesort
  + 执行: 如果值能完全放入buffer,则quicksort,如果不能,分批quicksort,最后mergeSort
  + 两种算法:
    + two passes(old): 读ID和排序字段,排序,最后根据id获取其他字段
        扫描表两次,速度慢,内存小
    + single pass(new): 读所需字段,排序
        扫描表两次,速度慢,内存大
  + 需要空间比预期的大,因为会使用字段的最大空间,比如varchar(255),会直接用255bit
  + 连表排序: 尽量让排序字段在第一个表
    1. 排序字段只在第一个表,先排序再join其他表, explain: Extra: Using Filesort;
    2. 排序字段不在第一个表,或者多表,先join到临时表,再排序, explain: Extra: Using temporary;
...
:wusd
查询执行引擎:
查询执行计划: 由parsing和optimizer产生,用于引擎执行查询计划.
handler: 代表一张表, 有列名和索引信息.优化器也使用handler来进行优化.
...
:wusd
返回结果给客户端: MySQL处理完最后一张表后,产生第一行数据,马上分包返回结果给Client.
好处:
1. server避免把结果保持在内存中
2. client尽快获取结果
...
:wusd
MySQL查询优化器的限制: 少部分情况MySQL优化的不好.

### UNION Limitations
union的外部限制条件,不会优化到内部

:wusd
UNION的限制: union外部的limit,不会优化到内部的表
```sql
...
:wusd
count()怎么干的?:
- count(columns...): count值,NULL值时不+1,有值时+1
- count(*): count行数,直接返回行数.
...
:wusd
更多复杂优化:
- 因为难优化,所以covering index: count需要扫描很多行,难以优化,covering index
- 缓存: 速度~简单~准确性,任选其二.
...
:wusd
优化连表查询:
- A表和B表在字段c连接时,哪个表有索引那个表被连.
- group by和order by的字段尽量在一张表内,这样会用索引排序,而不是filesort

### Optimizing GROUP BY with ROLLUP 无用
移除掉with ROLLUP
...
:wusd
优化limit和offset: 分页基本涉及排序,排序字段最好有索引.
